...
(
    ...
    // ------------------------------------------------------------
    // SVA (即時アサーション):
    // 最初のラウンド鍵 round_keys[0] は入力鍵そのものと一致すること
    // (Rust実装で w[0..3] が key をそのまま切り出したものである点に対応)
    // ------------------------------------------------------------
    always_comb begin
        for (int row = 0; row < 4; row++) begin
            for (int col = 0; col < 4; col++) begin
                assert (round_keys[0][row][col] == key[127 - (col*4 + row)*8 -: 8])
                    else $error("[SVA][key_expansion] round_keys[0][%0d][%0d]=%02h が鍵バイトと一致しません",
                                row, col, round_keys[0][row][col]);
            end
        end
    end
